...
Status RunOnce(const RunOptions& run_options,
  ...
  const uint64 RunOnce_Start = Env::Default()->NowMicros();
  ...
  *callable_options.mutable_run_options() = run_options;
  ...
  // up codes dont took too much time  ~10ms
  
  const uint64 RunOnce_Start_2 = Env::Default()->NowMicros();
  Session::CallableHandle callable_handle;
  TF_RETURN_IF_ERROR(session->MakeCallable(callable_options, &callable_handle)); //cost much time
  LOG(INFO) << "****Kkq:RunOnce real cost Took:"
            << GetLatencyMicroseconds(RunOnce_Start_2) << " microseconds."; 
  ...
  LOG(INFO) << "****Kkq:RunOnce Took:"
            << GetLatencyMicroseconds(RunOnce_Start) << " microseconds.";  
...
Status RunInitOp(const RunOptions& run_options, const string& export_dir,
  ...
  if (!init_op_name.empty()) {
    LOG(INFO) << "RunInitOp:Running initialization op on SavedModel bundle at path: "
              << export_dir;
    std::vector<std::pair<string, Tensor>> inputs;
...
Status LoadSavedModelInternal(const SessionOptions& session_options,
  ...
  const uint64 ReadMetaGraphDefFromSavedModel_start = Env::Default()->NowMicros();
  TF_RETURN_IF_ERROR(ReadMetaGraphDefFromSavedModel(export_dir, tags,
                                                    &bundle->meta_graph_def));
  LOG(INFO) << "***kkq:ReadMetaGraphDefFromSavedModel Took "
            << GetLatencyMicroseconds(ReadMetaGraphDefFromSavedModel_start) << " microseconds.";

  const uint64 ReadSavedModelDebugInfoIfPresent_start = Env::Default()->NowMicros();                                          
  TF_RETURN_IF_ERROR(
      ReadSavedModelDebugInfoIfPresent(export_dir, &bundle->debug_info));
  LOG(INFO) << "***kkqReadSavedModelDebugInfoIfPresent Took "
            << GetLatencyMicroseconds(ReadSavedModelDebugInfoIfPresent_start) << " microseconds.";  

  const uint64 LoadMetagraphIntoSession_start = Env::Default()->NowMicros();  
  TF_RETURN_IF_ERROR(LoadMetagraphIntoSession(
      session_options, bundle->meta_graph_def, &bundle->session));
  LOG(INFO) << "***kkqLoadMetagraphIntoSession Took "
            << GetLatencyMicroseconds(LoadMetagraphIntoSession_start) << " microseconds.";
  

  const uint64 RestoreSession_start = Env::Default()->NowMicros();
  TF_RETURN_IF_ERROR(RestoreSession(run_options, bundle->meta_graph_def,
                                    export_dir, &bundle->session));
  LOG(INFO) << "***kkqRestoreSession Took "
            << GetLatencyMicroseconds(RestoreSession_start) << " microseconds.";
...
Status RestoreSession(const RunOptions& run_options,
                      ...
                      std::unique_ptr<Session>* session) {
  //kkq:1)restoring graph from disk 

  const uint64 read_start_microseconds = Env::Default()->NowMicros();
  std::vector<AssetFileDef> asset_file_defs;
  TF_RETURN_IF_ERROR(internal::GetAssetFileDefs(meta_graph, &asset_file_defs)); //~2ms
  ...
  LOG(INFO) << "***kkq:RestoreSession: restoring graph from disk Took "<< restore_graph_walltime << " microseconds.";

  //kkq:2)graph init
  const uint64 graph_init_start_microseconds = Env::Default()->NowMicros();
  string init_op_name;
  TF_RETURN_IF_ERROR(
      internal::GetInitOp(export_dir, meta_graph, &init_op_name)); //1~2ms
  LOG(INFO) << "***kkq:RestoreSession:GetInitOp "<< GetLatencyMicroseconds(graph_init_start_microseconds) << " microseconds.";

  const uint64 RunInitOp_start_microseconds = Env::Default()->NowMicros();
  TF_RETURN_IF_ERROR(RunInitOp(run_options, export_dir, meta_graph,
                               asset_file_defs, session->get(), init_op_name));
  LOG(INFO) << "***kkq:RestoreSession:RunInitOp "<< GetLatencyMicroseconds(RunInitOp_start_microseconds) << " microseconds.";
